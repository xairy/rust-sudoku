...
impl App {
    ...
    pub fn on_render_new(&mut self, c: Context, g: &mut G2d, cache: &mut Glyphs) {
        clear([1.0; 4], g);

        let pointed_cell = field::Coords {
            x: (self.mouse_coords.x / self.settings.cell_size.x).floor() as u8,
        ...
        if let Some(ref cell) = self.selected_cell {
            if let Some(digit) = self.field.get_cell(cell.x, cell.y).digit {
                for y in 0..9 {
                    for x in 0..9 {
                        if let Some(other_digit) = self.field.get_cell(x, y).digit {
            ...
            for x in 0..9 {
                if let Some(ref digit) = self.field.cells[y][x].digit {
                    let transform = c.transform.trans(
                        (x as f64) * self.settings.cell_size.x + self.settings.text_offset.x,
                        (y as f64) * self.settings.cell_size.y + self.settings.text_offset.y,
                    );
                    let text = graphics::Text::new(self.settings.font_size);
                    text.draw(&digit.to_string(), cache, &c.draw_state, transform, g)
                        .unwrap();
                }
            }
        }
        ...
        for n in 1..9 {
            ...
            rectangle(
                ...
                [
                    0.0,
                    (n as f64) * self.settings.cell_size.y - thick / 2.0,
                    self.settings.wind_size.x,
                    thick / 2.0,
                ],
                c.transform,
                g,
